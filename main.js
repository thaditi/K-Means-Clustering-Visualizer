...
var height = Math.max(130, width * 0.5);
...
function make() {
  var circles = pointg.selectAll("circle").data(points);
  ...
  circles
    ...
    .attr("fill", function (d) {
      return d.cluster ? d.cluster.color : "black";
    })
    .attr("r", 5);

  if (points[0].cluster) {
    var l = lineg.selectAll("line").data(points);
    var updateLine = function (lines) {
      lines
        ...
        .attr("x2", function (d) {
          return d.cluster.center.x;
        })
        .attr("y2", function (d) {
          return d.cluster.center.y;
        })
  ...
  var c = centerg.selectAll("path").data(clusters);
  var centroidUpdate = function (centers) {
    centers
      .attr("transform", function (d) {
        return "translate(" + d.center.x + "," + d.center.y + ") rotate(0)";
      })
  ...
  centroidUpdate(c.transition().duration(500));
}
...
function updateGroups() {
  ...
  points.forEach(function (point) {
    ...
    clusters.forEach(function (g) {
      var d =
        Math.pow(g.center.x - point.x, 2) + Math.pow(g.center.y - point.y, 2);
      if (d < min) {
        min = d;
        cluster = g;
      }
